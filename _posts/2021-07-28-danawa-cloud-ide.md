...
여기까지 진행하면 터미널을 오픈할 수 있지만, 클릭 한 번으로 생성할 수 있도록 다음과 같은 코드를 작성합니다. `Docker Api`를 통해 URL로 호출하는 방식을 사용합니다.
...
가상 IDE를 만들고 사용하는 것은 위에서 설명드렸던 내용으로도 충분히 가능합니다. 하지만 `리버스 프록시`를 적용하면 포트 관리를 효율적으로 할 수 있고, 서브도메인, 터미널에서 구동한 프로젝트 실행 확인을 해볼 수 있어서 좋습니다. 전에 작성했던 내용을 참고 했습니다. [Traefik과 Docker를 활용한 Reverse Proxy 구축](https://danawalab.github.io/common/2021/07/14/traefik-reverse-froxy.html)
