...
- 5번의 테스트중 필터와 `function_score` 적용시 평균 소요시간이 증가했습니다. 쓰레드 8기준으로 FASTCAT의 성능은
평균 72ms, TPS 110.52였습니다.
...
## 5. 결론
색인,배포 테스트 때 ElasticSearch의 성능이 더 좋았던 것과 달리 검색에서는 반대로 ElasticSearch의 검색성능이 더 낮게 나왔습니다. 스코어의 계산을 뺄 수 없는 구조라 쿼리 튜닝 혹은 다른 방법으로 이를 개선 할 수 있는지 학인해야할 것 같고 이후 동적색인와 같이 검색을 수행 했을 때도 FASTCAT에 비해 어떤 이점과 보완할 점이 있는지 확인이 필요합니다.
