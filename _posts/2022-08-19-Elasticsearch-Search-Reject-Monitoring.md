...
ES에서 제공한 Reject 갯수를 확인하는 API 만으로는 한계가 있어서 Reject Exception의 실시간 모니터링 구현을 해 봤습니다.
Reject Exception의 지속적인 모니터링 시스템이 필요하신 분에게 참고가 되었으면 좋겠습니다.

Reject Exception이 발생한다는 것은 사용자가 요청한 결과를 제공하지 못했다는 의미입니다.
그만큼 심각한 에러의 한 종류이자 중요한 성능 지표입니다. 
대용량 처리 시스템을 ES로 설계하였다면 실시간으로 모니터링 할 필요성이 있습니다.
