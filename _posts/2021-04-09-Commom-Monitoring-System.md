...
제일 좋은 점은 데이터 분석할 때 모니터링 대시보드에서 시간 설정 만으로 가능 한 것입니다.
1년 동안 상품 수 증가와 검색 속도의 증가 추이를 한 번에 동시에 볼 수 있습니다.
그리고 신규 서비스 또는 항목을 모니터링 대상에 추가하고 싶다면 로그 기록과 간단한 설정 추가로 그라파나 대시보드에 시각화할 수 있습니다. 
기본적이고 핵심내용을 대시보드에 추가했지만, 좀 더 자세히 보고 싶을 때는 그라파나 패널을 추가해서 보면 데이터를 상세화 하면 편리합니다.
